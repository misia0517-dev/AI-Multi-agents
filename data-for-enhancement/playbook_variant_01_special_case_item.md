...
**Version 0.2 â€” Internal Operations Reference**
...
|---|---|---|---|---|---|
| C1_I95_NJ_BOS | NJ â†’ Boston (I-95) | Newark_NJ_DC | Boston_MA | Tier 1 | Existing corridor |
| C2_NJ_PHL | NJ â†’ Philadelphia | Newark_NJ_DC | Philadelphia_PA | Tier 2 | Added corridor for multi-region planning |
...
- Forecast window: Next 2 forecast days (`forecast_days = 2`) to cover â€œnext-dayâ€ planning
- Variables used:
...
Compute a corridor/day risk score first (0â€“3), then aggregate to a 48-hour corridor risk.
...
|---|---|---|
| Heavy Precipitation Risk | `precipitation_sum` | â‰¥ 15.0 mm/day |
| High Wind Risk | `wind_gusts_10m_max` | â‰¥ 45.0 km/h |
| Freezing Risk | `temperature_2m_min` | â‰¤ 0.0 Â°C |
...
## Appendix A â€” Item Master Appendix (Searchable)
...
Use this table when `item_id` is unknown/invalid in the master but can be reconciled to a canonical item. All legacy-mapped rows must be reported as â€œfixed via legacy mapping.â€
...
| 10040 | EPI-AI | SPECIAL_CASE | Executive test: epinephrine auto-injector is treated as the special-case review item for this variant |

### A.4 Substitution Policy (Operationally Allowed Cases)
Substitution is only allowed when explicitly permitted below. If not listed, do not substituteâ€”flag for investigation.
